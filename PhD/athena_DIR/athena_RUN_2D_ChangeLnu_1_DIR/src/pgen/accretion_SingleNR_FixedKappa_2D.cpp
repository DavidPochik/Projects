...
static Real T_eq, Ye_eq, dpdd_0, pModify, Mach;
...
//      std::cout << "qdot         = " << qdot << " erg/s/g" << std::endl;
    ...
    if(i==maxcsNR){
      std::cout << "(Newton-Raphson for T) Convergence failed. Switch to floating T..." << std::endl;
      Arr[0]=-1.0;
    }
  }
  return;
...
void Double_NR_NEW(Real Arr[], Real ra, Real rhoa) {
  ...
  for(int i=0; i<=maxcdNR; i++){
    ...
    // Define scale terms
    Real H0         = Heating_QW_Modified(x_0, Y0);
    Real Source0    = YeTejas_Source(T0*kbol_MeV, Y0, x_0, rhoa);
    
    // Define 'original' source functions
    Real qdot    = Heating_QW_Modified(x_0, Y0) - Cooling_QW_Modified(rhoa, T0*kbol_MeV, Y0);
    Real Yedot   = YeTejas_Source(T0*kbol_MeV, Y0, x_0, rhoa) - YeTejas_Sink(T0*kbol_MeV, Y0, x_0, rhoa);

    // Define source functions with differentials
    Real dqdot_T  = Heating_QW_Modified(x_0, Y0) - Cooling_QW_Modified(rhoa, (T0+deltaT0)*kbol_MeV, Y0);
    Real dqdot_Y  = Heating_QW_Modified(x_0, Y0+deltaY0) - Cooling_QW_Modified(rhoa, T0*kbol_MeV, Y0+deltaY0);
    Real dYedot_T = YeTejas_Source((T0+deltaT0)*kbol_MeV, Y0, x_0, rhoa) - YeTejas_Sink((T0+deltaT0)*kbol_MeV, Y0, x_0, rhoa);
    Real dYedot_Y = YeTejas_Source(T0*kbol_MeV, Y0+deltaY0, x_0, rhoa)   - YeTejas_Sink(T0*kbol_MeV, Y0+deltaY0, x_0, rhoa);
    ...
    if(i==maxcdNR) {
      std::cout << "Double NR failed. Returning guess values..." << std::endl;
      Arr[0] = tg; //T1;
      Arr[1] = yg; //Y1;
    }
  }
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  Mach     = pin->GetOrAddReal("problem","MachNumber",1.3);
  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        if(X_n < 1.0 && r>5.0e6) {
          vdYedr = ExpSupp * X_n * YeTejas(temp, Ye_i, x, rho);
          qdot   = ExpSupp * X_n * (Heating_QW_Modified(x, Ye_i) - Cooling_QW_Modified(rho, temp, Ye_i)); // erg/s/g
        } else {
          vdYedr = ExpSupp * YeTejas(temp, Ye_i, x, rho);
          qdot   = ExpSupp * (Heating_QW_Modified(x, Ye_i) - Cooling_QW_Modified(rho, temp, Ye_i)); // erg/s/g
        }
...
void InflowInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=1; i<=ngh; ++i) {
          prim(IDN,k,j,is-i) = rho_0; // dont update rho for 2D
    ...
    //      Real Yeq = Arr[1];
          ...
          pmb->pscalars->r(ye_index,k,j,is-i) = ye_a; //Yeq;
          // If Single_NR fails to converge, switch to floating.
          if (Teq > 0.0) {
            pmb->pscalars->r(t_index,k,j,is-i) = Teq; //pmb->pscalars->r(t_index,k,j,is); //Teq; 
//          pmb->pscalars->r(t_index,k,j,is-i) = pmb->pscalars->r(t_index,k,j,is);
          } else {
            pmb->pscalars->r(t_index,k,j,is-i) = pmb->pscalars->r(t_index,k,j,is);
          }
          Real T_g = pmb->pscalars->r(t_index,k,j,is-i);
          Real r_scalar[nscalar_size];
          Real s_scalar[nscalar_size];
          for (int ns=0; ns<nscalar_size; ns++) {
          ...
          if (NON_BAROTROPIC_EOS) {
            prim(IPR,k,j,is-i) = pmb->peos->PresFromRhoT(rho_0, T_g, r_scalar); //This one works for tau
          }
      }
    }
  }
    ...
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=1; i<=ngh; ++i) {
        if (prim(IVX,k,j,ie) < 0.0) {
          ...
          prim(IDN,k,j,ie+i) = rho_f; //prim(IDN,k,j,ie);
          prim(IVX,k,j,ie+i) = v_f;  //prim(IVX,k,j,ie);
          prim(IVY,k,j,ie+i) = 0.0;
          prim(IVZ,k,j,ie+i) = 0.0;
        ...
        if (NON_BAROTROPIC_EOS) {
          ...
          Real etot          = pmb->peos->EtotFromRhoT(prim(IDN,k,j,ie+i),pmb->pscalars->r(t_index,k,j,ie+i),r_scalar);
          Real lightspeed    = 2.99792458e10;              // Speed of light in cm/s
          prim(IPR,k,j,ie+i) = rho_f * (etot + std::pow(lightspeed,2.0)) / ((gamma * std::pow(Mach * lightspeed, 2.0))/(std::pow(v_f, 2.0)) - 1.0);
  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
     ...
     //   Real temp     = peos->TFromRhoP(phydro->w(IDN,k,j,i),phydro->w(IPR,k,j,i),efrac); // units of K
        ...
        if(X_n<1.0 && r>5.0e6) {
          user_out_var(1,k,j,i) = ExpSupp * X_n * (Heating_QW_Modified(x, ye) - Cooling_QW_Modified(rho, temp_MeV, ye));
        } else {
          user_out_var(1,k,j,i) = ExpSupp * (Heating_QW_Modified(x, ye) - Cooling_QW_Modified(rho, temp_MeV, ye));
        }
        // Sound Speed
        ...
        // Tau diagnostics
        user_out_var(12,k,j,i) = r;   // Radius
        user_out_var(13,k,j,i) = rho; // Density
        user_out_var(14,k,j,i) = ye;  // Electron fraction
        //user_out_var(15,k,j,i) = 0.0; //
      }
    }
  }
}
