...
// OutData has length 9 (HelmholtzConstants::nOut)
  ...
  if ((var<=0.0) || (rho<=0.0)) {
      ...
      printf("ERR (%s): %.4e, rho: %.4e,  Temp: %.4e \n", varnames[index],var,rho,
             GuessTemp);
      std::cout << "e * rho       = " << OutData(0) << " erg/cm^3"   << std::endl;
      std::cout << "denerdt * den = " << OutData(1) << "erg/cm^3/s?" << std::endl;
      std::cout << "Cs^2          = " << OutData(4) << "cm^2/s^2"    << std::endl;
      std::cout << "dpres/drho    = " << OutData(6) << "erg/g"       << std::endl;
      std::cout << "eta_ele       = " << OutData(7) << std::endl;
      std::cout << "gam1          = " << OutData(8) << std::endl;
...
Real EquationOfState::GammaFromRhoT(Real rho, Real T, Real* r) {
  Real ye = fixed_ye;
  Real abar = fixed_abar;
  if (NSCALARS > 0 && i_ye >= 0) {
    ye = r[i_ye];
...
Real EquationOfState::EtotFromRhoT(Real rho, Real T, Real* r) {
  Real ye = fixed_ye;
  Real abar = fixed_abar;
  if (NSCALARS > 0 && i_ye >= 0) {
    ye = r[i_ye];
...
void EquationOfState::InitEosConstants(ParameterInput *pin) {
  ...
  phelm->HelmLookupRhoT(1e12, 4.64e11,0.05,1.0, EosData);
