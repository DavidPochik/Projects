...
if(rank==0):
	...
	def averaging():
		...
		for i in range(0,nR):
			for k in range(0,nT):
				for j in range(0,nTheta-1):
					...
					Mdot_avg_theta[k,i]  += (1.0 / 4.0) * delta_Theta * (4.0 * np.pi) * \
											    (rho_array[k,j,i]   * r[i]**2 * np.abs(vr_array[k,j,i]) + \
											     rho_array[k,j+1,i] * r[i]**2 * np.abs(vr_array[k,j+1,i])) 
		...
		return[rho_avg_t, vr_avg_t, S_avg_t, Ye_avg_t, T_avg_t, P_avg_t, cs_avg_t, qdot_avg_t, gamma_avg_t, \
		       rho_avg_theta, vr_avg_theta, S_avg_theta, Ye_avg_theta, T_avg_theta, P_avg_theta, cs_avg_theta, qdot_avg_theta, gamma_avg_theta, Mdot_avg_theta, \
	...
	def BruntVaisaila_Chi_of_t(rho_avg, vr_avg, T_avg, P_avg, S_avg, qdot_avg, ye_avg, time):
		...
		return[Chi, Chi_t_avg_array, Chi_t_avg, rgain, rshock]
	...
	[Chi, Chi_t_avg, Chi_t_avg_value] = BruntVaisaila_Chi_of_t(rho_avg_theta, vr_avg_theta, T_avg_theta, P_avg_theta, S_avg_theta, qdot_avg_theta, Ye_avg_theta, time)
	...
	bx2       = fig.add_axes([0.005,0.1,0.015,0.72])
	cx2       = fig.add_axes([0.93,0.1,0.015,0.72])
	...
	rmin = min(data1['x1v']) / 1.0e5 # min radius in km
	rmin = max(data1['x1v']) / 1.0e5 # max radius in km
	# Density
	...
	Tplotmin   = np.min(minValues)
	Tplotmax   = np.max(maxValues)
	fig_tau    = plt.figure()
	axT        = plt.axes(xlim=(rmin,rmax), ylim=(Tplotmin, Tplotmax),xlabel=r"$R$ [km]",ylabel=r"Mach number")
	line1a     = ax.semilogx([],   [], lw=1.75, color=l1acolor, label=r'$\tau_{\mathrm{adv}}$')[0]
	line1b     = ax.semilogx([],   [], lw=1.75, color=l1bcolor, linestyle='dashed', label=r'$\tau_{\mathrm{heat}}$')[0]	
	...
	print(arrT)
	animM.save(name+'_Tadv_Theat.gif', writer='imagemagick')
	...
	def animate_Gamma_T(i):
		...
		print('max(gamma) = ' + str(np.amax(Gamma)))
		print('max(T)     = ' + str(np.amax(T)))
		print('min(gamma) = ' + str(np.amin(Gamma)))
		print('min(T)     = ' + str(np.amin(T)))
	...
	def animate_Multi(i):
		...
		for j in range(0,len(lines)):
			...
			if(j==1):
				lines[j].set_data(r/1.0e5,T_avg_theta[i,:]*kb)
			if(j==2):
			...
			if(j==5):
				lines[j].set_data(r/1.0e5,Mdot_avg_theta[i,:])
			if(j==6):
