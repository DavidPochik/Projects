...
if(rank==0):
	...
	def averaging():
		...
		for i in range(0,nR):
			for k in range(0,nT):
				for j in range(0,nTheta):
					delta_Theta            = thetaf[j+1] - thetaf[j]
					rho_avg_theta[k,i]    += (1.0 / 2.0) * delta_Theta * (rho_array[k,j,i]    * np.sin(thetaf[j]))
					vr_avg_theta[k,i]     += (1.0 / 2.0) * delta_Theta * (vr_array[k,j,i]     * np.sin(thetaf[j]))
					vtheta_avg_theta[k,i] += (1.0 / 2.0) * delta_Theta * (vtheta_array[k,j,i] * np.sin(thetaf[j]))
					S_avg_theta[k,i]      += (1.0 / 2.0) * delta_Theta * (S_array[k,j,i]      * np.sin(thetaf[j]))
					Ye_avg_theta[k,i]     += (1.0 / 2.0) * delta_Theta * (Ye_array[k,j,i]     * np.sin(thetaf[j]))
					T_avg_theta[k,i]      += (1.0 / 2.0) * delta_Theta * (T_array[k,j,i]      * np.sin(thetaf[j]))
					P_avg_theta[k,i]      += (1.0 / 2.0) * delta_Theta * (P_array[k,j,i]      * np.sin(thetaf[j]))
					cs_avg_theta[k,i]     += (1.0 / 2.0) * delta_Theta * (cs_array[k,j,i]     * np.sin(thetaf[j]))
					qdot_avg_theta[k,i]   += (1.0 / 2.0) * delta_Theta * (qdot_array[k,j,i]   * np.sin(thetaf[j]))
					gamma_avg_theta[k,i]  += (1.0 / 2.0) * delta_Theta * (gamma_array[k,j,i]  * np.sin(thetaf[j]))
		...
		for i in range(0,nR):
			for j in range(0,nTheta):
				delta_Theta           = thetaf[j+1] - thetaf[j]
				rho_avg_t_theta[i]   += (1.0 / 2.0) * delta_Theta * (rho_avg_t[j,i]   * np.sin(thetaf[j]))
				vr_avg_t_theta[i]    += (1.0 / 2.0) * delta_Theta * (vr_avg_t[j,i]    * np.sin(thetaf[j]))
				S_avg_t_theta[i]     += (1.0 / 2.0) * delta_Theta * (S_avg_t[j,i]     * np.sin(thetaf[j]))
				Ye_avg_t_theta[i]    += (1.0 / 2.0) * delta_Theta * (Ye_avg_t[j,i]    * np.sin(thetaf[j]))
				T_avg_t_theta[i]     += (1.0 / 2.0) * delta_Theta * (T_avg_t[j,i]     * np.sin(thetaf[j]))
				P_avg_t_theta[i]     += (1.0 / 2.0) * delta_Theta * (P_avg_t[j,i]     * np.sin(thetaf[j]))
				cs_avg_t_theta[i]    += (1.0 / 2.0) * delta_Theta * (cs_avg_t[j,i]    * np.sin(thetaf[j]))
				qdot_avg_t_theta[i]  += (1.0 / 2.0) * delta_Theta * (qdot_avg_t[j,i]  * np.sin(thetaf[j]))
				gamma_avg_t_theta[i] += (1.0 / 2.0) * delta_Theta * (gamma_avg_t[j,i] * np.sin(thetaf[j]))
	...
	def BruntVaisaila_Chi_of_t(rho_avg, vr_avg, T_avg, P_avg, S_avg, qdot_avg, ye_avg, time):
		...
		for j in range(0,nT):
			...
			for i in range(imod,int(nR)-imod):
				if(Mdot[j,i]==MdotMax[j]):
					rshock[j]   = r[i]
					i_rshock[j] = int(i)
					print('rshock[j] = ' + str(rshock[j]/1.0e5) + ' km')
					print('time[j]   = ' + str(time[j]) + ' s')
					print('j (time)  = ' + str(j))
					print('i (space) = ' + str(i))
					print('imod      = ' + str(imod))
					print(' ')
		...
		for j in range(0,nT):
			...
			if(Chi[j]>10.0 and rshock[j]/1.0e5>400.0):
				print('(2D Chi calculation script) Chi>10 and rshock>400km achieved, assuming explosion. Cut off time axis here:')
				print('Time[j]   = ' + str(time[j]))
				print('j         = ' + str(j))
				print('rshock[j] = ' + str(rshock[j]/1.0e5) + ' km')
				print('Chi[j]    = ' + str(Chi[j]))
				iEndTime = j
				break
		...
		Rshock_avg_t = 0.0
		Rgain_avg_t  = 0.0
		DELTA_T      = time[int(iEndTime)] - time[0]
	#	for j in range(0,nT-1):
		for j in range(0,int(iEndTime)-1):
		...
		return[Chi, Chi_t_avg_array, Chi_t_avg, rgain, rshock, Rshock_avg_array, Rgain_avg_array, iEndTime]
	...
	rho_avg_t_theta, vr_avg_t_theta, S_avg_t_theta, Ye_avg_t_theta, T_avg_t_theta, P_avg_t_theta, cs_avg_t_theta, qdot_avg_t_theta, gamma_avg_t_theta] = \
	averaging()	
	[Chi, Chi_t_avg, Chi_t_avg_value, Rgain, Rshock, Rshock_avg_t, Rgain_avg_t, iEndTime] = \
	...
	# LHS, entropy
	min_S = np.amin(S_array) + 0.1 * np.amin(S_array)
	max_S = np.amax(S_array) + 0.3 * np.amax(S_array)
	# RHS, vr
	...
	plt.figure(0)
	plt.clf()
	plt.plot(time[0:iEndTime],Chi[0:iEndTime],linewidth=2.0,linestyle='solid',color='black',label=r'$\left< \chi \right>_{\theta}$')
	plt.plot(time[0:iEndTime],Chi_t_avg[0:iEndTime],linewidth=1.5,linestyle='dashed',color='red',label=r'$\left< \chi \right>_{\theta,t}$')
	plt.xlabel(r'$t$ [s]')
	...
	plt.figure(0)
	plt.clf()
	plt.plot(time[0:iEndTime],Rshock[0:iEndTime],linewidth=2.0,linestyle='solid',color='black',label=r'$\left< R_{\mathrm{shock}} \right>_{\theta}$')
	plt.plot(time[0:iEndTime],Rgain[0:iEndTime], linewidth=2.0,linestyle='solid',color='red',  label=r'$\left< R_{\mathrm{gain}}  \right>_{\theta}$')
	plt.plot(time[0:iEndTime],Rshock_avg_t[0:iEndTime],linewidth=1.5,linestyle='dashed',color='black',label=r'$\left< R_{\mathrm{shock}} \right>_{\theta,t} = $' + \
		str(Rshock_avg_t[0]/1.0e5) + ' km')
	plt.plot(time[0:iEndTime],Rgain_avg_t[0:iEndTime], linewidth=1.5,linestyle='dashed',color='red',  label=r'$\left< R_{\mathrm{gain}}  \right>_{\theta,t} = $' + \
