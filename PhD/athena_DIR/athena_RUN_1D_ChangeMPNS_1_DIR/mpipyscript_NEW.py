...
e_nue       = float(sys.argv[11])
rMax_set    = float(sys.argv[12])
Lnu         = float(sys.argv[13])
R_PNS       = float(sys.argv[14])
M_PNS       = float(sys.argv[15]) # in Msun/s
...
e      = np.zeros(shape=(upper_ind-lower_ind,nR))
...
for i in range(lower_ind,upper_ind):
	...
	e[i-lower_ind][:]      = (EOSData['etot'])[0:nR]             # erg/g
...
for i in range(0,iSize):
	...
	comm.Barrier()
	e_gather      = comm.gather(e[i,0:nR],root=0)
	comm.Barrier()
...
if(rank==0):
	...
	e_gather_T      = np.array(e_gather_T)
	...
	for i in range(0,iSize):
		for j in range(0,nR):
			for l in range(0,size):
				...
				e_array[l+size*i][j]      = e_gather_T[i][l][j]
	...
	def fermi(n, eta):
		if(n==0):
			fermi = np.log(1.0 + np.exp(eta))
			return fermi
		elif(n==1):
			a  = np.exp(-1.0 * np.abs(eta))
			s  = pow(eta,2) / 2.0 + 1.6449341
			ff = a - pow(a,2) / 4.0 + pow(a,3) / 9.0 - pow(a,4) / 16.0 + pow(a,5) / 25.0 - pow(a,6) / 36.0 + pow(a,7) / 49.0
			if(eta < 0):
				fermi = ff
				return fermi
			elif(eta == 0):
			...
			ff = 2.0 * (a - pow(a,2) / 8.0 + pow(a,3) / 27.0 - pow(a,4) / 64.0 + pow(a,5) / 125.0 - pow(a,6) / 216.0)
			if(eta<0):
				fermi = ff
				return fermi
			elif(eta==0):
			...
			a  = np.exp(-1.0 * np.abs(eta))
			s  = pow(eta,4) / 4.0 + 4.9348022 * pow(eta,2) + 11.351273
			ff = 6.0 * (a - pow(a,2) / 16.0 + pow(a,3) / 81.0 - pow(a,4) / 256.0)
			if (eta<0):
				fermi = ff
				return fermi
			elif(eta==0):
			...
			ff = 24.0 * (a - pow(a,2) / 32.0 + pow(a,3) / 243.0)
			if (eta<0):
				fermi = ff
				return fermi
			elif(eta==0):
			...
			a  = np.exp(-1.0 * np.abs(eta))
			s  = pow(eta,6) / 6.0 + 8.2246703 * pow(eta,4) + 113.64394 * pow(eta,2) + 236.53226
			ff = 120.0 * (a - pow(a,2) / 64.0 + pow(a,3) / 729.0)
			if(eta<0):
				fermi = ff
				return fermi
			elif(eta==0):
			...
			print('n isnt between 0 and 5. PROBLEM! Returning zero.')
			return 0.0
...
#		rshock     = np.zeros(nT)
#		i_rshock   = np.zeros(nT)
#		drhodr     = np.zeros(shape=(nT,nR))
#		drhodr_max = np.zeros(nT)
#		for j in range(0,nT):
#			for k in range(0,nR):
#				if(k==0):
#					drhodr[j,k] = np.abs(rho_avg[j,k+1] - rho_avg[j,k])   / np.abs(r[k+1] - r[k])
#				elif(k==nR-1):
#					drhodr[j,k] = np.abs(rho_avg[j,k]   - rho_avg[j,k-1]) / np.abs(r[k]   - r[k-1]) 
#				else:
#					drhodr[j,k] = np.abs(rho_avg[j,k+1] - rho_avg[j,k-1]) / np.abs(r[k+1] - r[k-1])	
#			drhodr_max[j] = np.max(drhodr[j,imod:int(nR)-imod])
#			for l in reversed(range(imod,nR-imod)):
#				if(drhodr[j,l]==drhodr_max[j]):
#					rshock[j]   = r[l]
#					i_rshock[j] = int(l)

		dMdotdR = np.zeros(shape=(nT,nR))
		for j in range(0,nT):
			for k in range(0,nR):
				if(k==0):
					dMdotdR[j,k] = np.abs((Mdot[j,k+1] - Mdot[j,k])   / (r[k+1] - r[k]))
				elif(k==nR-1):
					dMdotdR[j,k] = np.abs((Mdot[j,k]   - Mdot[j,k-1]) / (r[k]   - r[k-1]))
				else:
					dMdotdR[j,k] = np.abs((Mdot[j,k+1] - Mdot[j,k-1]) / (r[k+1] - r[k-1])) 

		# I adjust the endpoints by imod so that the maximum picks out the shock and not the endpoint behavior
		maxdMdotdR = np.zeros(nT)
		for j in range(0,nT):
			maxdMdotdR[j] = np.max(dMdotdR[j,imod:int(nR)-imod])

		rescaled_dMdotdR = np.zeros(shape=(nT,nR-imod*2))
		for j in range(0,nT):
			for k in range(imod,nR-imod):
				rescaled_dMdotdR[j,k-imod] = dMdotdR[j,k] / maxdMdotdR[j]

		i_rshock = np.zeros(nT)
		rshock   = np.zeros(nT)
		for j in range(0,nT):
			for k in reversed(range(0,nR-imod*2)):
				if(rescaled_dMdotdR[j,k]>tol):
					i_rshock[j] = k
					rshock[j]   = r[k]
					break
				elif(k==0):
					print('Shock finding script has probably failed (oh no).')
	...
	def OpticalDepth(r, rf, rho, ye, eps_nue, nT):
		nR          = np.size(r)	
		sigma_0     = 1.76e-44           # weak cross section, cm^2
		alpha       = -1.26              # axial vector
		mcsq        = 0.511              # electron rest mass in MeV
		delta_np    = 1.293              # neutron-proton mass difference, MeV
		Na          = 6.022e23           # Avogadro's number, g^-1	
		fermi2      = fermi(2,0)
		fermi3      = fermi(3,0)
		fermi4      = fermi(4,0)
		energy_term = ((eps_nue * fermi2 / fermi3)**2 * fermi4 / fermi2 + 2.0 * delta_np * (eps_nue * fermi2 / fermi3) * fermi3 / fermi2 + delta_np**2) /\
				mcsq**2
		sigma_nue_n = sigma_0 * ((1.0 + 3.0 * alpha**2) / 4.0) * energy_term
		Tau         = np.zeros(nT)
		for j in range(0,nT):
			integral = 0.0
			for i in range(0,nR):
				dR        = rf[i+1] - rf[i]
				integral += Na * sigma_nue_n * dR * (1.0 - ye[j,i]) * rho[j,i]

			Tau[j] = integral

		return [Tau]

	def Bernoulli_integral(r, v, epsilon, P, rho):
		G     = 6.6743*pow(10,-8)          # cm^3 g^-1 s^-1
		M     = M_PNS * 1.989e33           # g
		mu    = G*M
		Bern  = np.zeros(nR)
		...
		for i in range(0,nR):
			...
			Pterm[i] = P[i] / rho[i]
			Gterm[i] = mu / r[i]
			Bern[i]  = vterm[i] + eterm[i] + Pterm[i] - Gterm[i] #v[i]**2 / 2.0 + epsilon[i] + P[i] / rho[i] - mu / r[i]
		return[Bern, vterm, eterm, Pterm, Gterm]
	...
	[tau_nu] = OpticalDepth(R, Rf, rho_array, Ye_array, e_nue, nT)
	[Bint, B_v, B_e, B_P, B_G] = Bernoulli_integral(r, vr_array[nT-1,:], e_array[nT-1,:], P_array[nT-1,:], rho_array[nT-1,:])
...
#	plt.legend()
	...
	plt.figure(0)
	plt.clf()
	plt.plot(time,tau_array,linewidth=2.0,linestyle='solid',color='blue', label=r'Code')
	plt.plot(time,tau_nu,   linewidth=2.0,linestyle='dashed',color='red', label=r'Post process')
	...
	print('######## Generating Bernoulli plots #########')
	plt.figure(0)
	plt.clf()
	for i in range(0,nR):
		if(r[i]/1.0e5>rMax_set):
			i_rMax = i
			print('max radius in plot: '+str(r[int(i_rMax)]/1.0e5)+' km')
			print('i_rMax = ' + str(i_rMax))
			break
	name = 'Bernoulli_terms_Mach_'+str(Mach_set)+'_Mdot_'+str(Mdot_set)	
	plt.plot(r[0:int(i_rMax)]/1.0e5,B_v[0:int(i_rMax)],'black',linewidth=2.0,label=r'$1/2 v^2$')
	plt.plot(r[0:int(i_rMax)]/1.0e5,B_e[0:int(i_rMax)],'green',linewidth=2.0,label=r'$\epsilon$')
	plt.plot(r[0:int(i_rMax)]/1.0e5,B_P[0:int(i_rMax)],'red',linewidth=2.0,label=r'$P/\rho$')	
	plt.plot(r[0:int(i_rMax)]/1.0e5,B_G[0:int(i_rMax)],'magenta',linewidth=2.0,label=r'$GM/r$')
	plt.legend()
	plt.xlabel(r'$R$ [km]')
	plt.ylabel(r'[erg g$^{-1}$]')
	plt.yscale('log')
	plt.title(r'$\mathcal{M}=$'+str(Mach_set)+r', $\dot{M}$='+str(Mdot_set)+r' M$_{\odot}$ s$^{-1}$, $L_{\nu}$='+str(Lnu)+r' erg s$^{-1}$, $t=$'+str(time[nT-1])+' s, $R_{\mathrm{PNS}}=$'+str(R_PNS)+' km')
	plt.savefig(name+'.png',bbox_inches='tight')	
	plt.clf()
	plt.close()
	

	zero_array = np.zeros(nR)
	plt.figure(0)
	plt.clf()
	name = 'Bernoulli_integral_Mach_'+str(Mach_set)+'_Mdot_'+str(Mdot_set)	
	plt.plot(r[0:int(i_rMax)]/1.0e5,Bint[0:int(i_rMax)]/1.0e19,'black',linewidth=2.0)
	plt.plot(r[0:int(i_rMax)]/1.0e5,zero_array[0:int(i_rMax)],'red',linestyle='dashed',linewidth=1.25)
	plt.xlabel(r'$R$ [km]')
	plt.ylabel(r'$\mathcal{B}$ [$10^{19}$ erg g$^{-1}$]')
	#plt.title(r'$\mathcal{M}=$'+str(Mach_set)+r', $\dot{M}$='+str(Mdot_set)+r' M$_{\odot}$ s$^{-1}$, $L_{\nu}$='+str(Lnu)+r' erg s$^{-1}$, $t=$'+str(time[nT-1])+' s')
	plt.title(r'$\mathcal{M}=$'+str(Mach_set)+r', $\dot{M}$='+str(Mdot_set)+r' M$_{\odot}$ s$^{-1}$, $L_{\nu}$='+str(Lnu)+r' erg s$^{-1}$, $t=$'+str(time[nT-1])+' s, $R_{\mathrm{PNS}}=$'+str(R_PNS)+' km')
	plt.savefig(name+'.png',bbox_inches='tight')	
	plt.clf()
	plt.close()
