...
//      std::cout << "qdot         = " << qdot << " erg/s/g" << std::endl;
    ...
    if(i==maxcsNR){
      std::cout << "(Newton-Raphson for T) Convergence failed. Switch to floating..." << std::endl;
      Arr[0]=-1.0; //T0;
    }
  }
  return;
...
//          pmb->pscalars->r(t_index,k,j,is-i) = pmb->pscalars->r(t_index,k,j,is);
            ...
            pmb->pscalars->r(t_index,k,j,is-i) = pmb->pscalars->r(t_index,k,j,is);
            Teq = pmb->pscalars->r(t_index,k,j,is);
          }
//          pmb->pscalars->r(t_index,k,j,is-i) = pmb->pscalars->r(t_index,k,j,is);
...
//        Real r_i     = pcoord->x1v(i);
       ...
       // local_sum    += 0.5 * deltaR * sigma_nue_n * Na * ((Chi_ip1 * rho_ip1) + (Chi_i * rho_i));
        local_sum   += deltaR * sigma_nue_n * Na * (1.0 - ye_i) * rho_i;
      }
    }
  } 
...
//        std::cout << "rho (u)  = " << phydro->u(IDN,k,j,i) << " g/cm^3" << std::endl;
        ...
        user_out_var(15,k,j,i) = sigma_0 * ((1.0 + 3.0 * std::pow(g_a,2)) / (4.0)) * std::pow(((eps_nue + delta_np) / (mcsq)),2); // Electron neutrino x-section
      }
    }
  }
}
